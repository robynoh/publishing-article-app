                        ...
                        <tbody>
                          @foreach($contactlists as $contactlist) 
                            ...
                            <input type="checkbox" name="contacts[]" class="input border mr-2" value="{{$contactlist->phone}}" >
                                ...
                                <input type="hidden" name="name[]" class="input border mr-2" value="{{$contactlist->name}}" >
...
<td>  <a href="javascript:void(0);" onclick="show_file_box();" class="button w-60 mr-2 mb-2 flex items-center justify-center bg-theme-14"> <i data-feather="file" class="w-4 h-4 mr-2"></i> Upload Excel Sheet </a>
        ...
        <br/>
        <form action="/users/multiplenamecontact/{{$lists}}" method="POST">
        {{ csrf_field() }}
  ...
  <thead>
       ...
       <tr>
<td><input name="name[]" type="text" class="input w-full border col-span" placeholder="Name" required>
...
function senddel(){
    ...
    window.location="/users/contactname/delete/"+$('#app_id').val();
