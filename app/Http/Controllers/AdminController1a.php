...
class AdminController extends Controller
{
    ...
    public static function registerrider()
    {
       $response=array();
       
       $file1=request()->file('profilephoto');
       $file2=request()->file('nationalid');
       $file3=request()->file('permit');
   ...
   $pilot->status=0;
   $pilot->pilotID=AdminController::random_number_with_dupe();
   $pilot->online_status=0;
   $pilot->verification_code= AdminController::confirmation_code();
...
return redirect()->back()->withSuccess('You have assigned a new pilot to this trip succesfully');  
    ...
    public static  function sendverificationcode($pilotid){ 
        ...
        $msg=urlencode($vericode->verification_code);
		 $url='https://www.bulksmsnigeria.com/api/v1/sms/create?api_token=ZR3eJEIlCdsHh3mcchZy38OX20qNzlpyklN4ZfBTjbNgQZTRZ0yJiwzaOqX2&from=Troopa&to='.urlencode($vericode->phone).'&body='.$msg.'&dnd=2';
         $ch = curl_init();
